...
通常无需额外设置。RTSS 会按共享内存槽位顺序拼接各应用的 OSD 文本；使用 OverlayEditor 自定义布局时，如果布局末尾留下了绝对光标位置，THRM 的转速文本可能继承该位置，表现为与最后一项一起移动。

仅在出现上述情况时，可以在 OverlayEditor 中为 THRM 添加一个定位锚点：

1. 在当前布局末尾添加一个图层，并命名为 `THRM Anchor`。
2. 保持图层文本为空，并将文字大小设置为 `1%`。
3. 将它移动到希望显示转速的位置，确认该图层位于图层列表最末尾，然后保存布局。

锚点图层生成的 `<P=x,y>` 标签会把 RTSS 光标移动到指定坐标，随后拼接的 `Cooler Fan` 文本便从该位置开始渲染。`1%` 的文字大小用于减小空白占位；THRM 会在转速文本前显式复位颜色和字号，因此转速文本仍按默认颜色和 `100%` 字号渲染。以后新增其他图层时，应继续让 `THRM Anchor` 保持在列表末尾。
