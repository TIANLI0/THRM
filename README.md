...
#### OverlayEditor 自定义布局兼容（可选）

通常无需额外设置。RTSS 会按共享内存槽位顺序拼接各应用的 OSD 文本；使用 OverlayEditor 自定义布局时，如果布局末尾留下了绝对光标位置或间距，THRM 的转速文本可能继承该状态，表现为与最后一项一起移动或上方出现空白。

仅在出现上述情况时，可以在 OverlayEditor 中为 THRM 添加一个定位锚点：

1. 先在 THRM 的“系统设置”中关闭“RTSS 屏幕显示”。
2. 打开 RTSS OverlayEditor，在当前布局的图层列表末尾添加一个文字图层。
3. 保持图层文本为空，将文字大小设置为 `1%`，并移动到希望显示转速的位置，然后保存布局。
4. 返回 THRM，重新打开“RTSS 屏幕显示”。

锚点图层的名称可以自定，例如 `THRM Anchor`。它生成的 `<P=x,y>` 标签会把 RTSS 光标移动到指定坐标，随后拼接的 `Cooler Fan` 文本便从该位置开始渲染。`1%` 的文字大小用于减小空白占位；THRM 会在转速文本前显式复位颜色和字号，因此转速文本仍按默认颜色和 `100%` 字号渲染。以后新增其他图层时，应继续让该锚点图层保持在列表末尾。
