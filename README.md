...
相比只提供固定挡位的基础控制方式，THRM 提供了可编辑风扇曲线、自适应学习（含无需画曲线的 2.0 自动模式）、功耗预测前馈、分时方案、噪音校准、RGB 灯效、温度历史与诊断导出等增强能力。
...
* 在线辨识本机散热特性（空载基线、各转速的每瓦温升），不依赖任何用户曲线
* 只需拖一个「安静 ↔ 低温」倾向滑块，目标温度、响应速度、可用转速区间等参数全部自动派生
* 由热模型与倾向解算出完整曲线，并随学习持续修正
* 结合麦克风噪音校准结果做「降温收益 vs 噪音代价」权衡
* 与倾向无关的安全红线温度，达到即全速
* 模型样本不足时自动退回保守内置曲线，可随时关闭回到手动曲线

### SmartControl 智能控温（1.0）
...
刷新频率只限制 THRM 写入 RTSS 共享内存的频率，不会改变散热器数据采样、风扇曲线或智能控温的响应速度。默认 1 秒适合大多数场景。
...
转速默认显示在现有 RTSS 项目下方，一般无需额外设置。需要调整时，可以在 THRM 的“转速显示位置”中选择：

* **在 THRM 中调整**：使用方向按钮或键盘移动转速，不修改 RTSS 布局。
* **在 RTSS 中调整**：使用 RTSS OverlayEditor 管理位置。若要单独拖动转速，请先关闭 OverlayEditor，再由 THRM 配置定位图层；修改前会自动备份当前布局。配置完成后，重新打开 OverlayEditor，拖动定位图层并保存即可。

两种方式可以随时切换，已有的位置设置会保留。
