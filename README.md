...
#### 配合 OverlayEditor 自定义布局定位

RTSS 会按共享内存槽位顺序拼接各应用的 OSD 文本。使用 OverlayEditor 自定义布局时，如果布局末尾是带坐标的图层，THRM 的转速文本可能继承该图层的位置，表现为与最后一项一起移动。

可以在 OverlayEditor 中为 THRM 添加一个独立锚点：

1. 在当前布局末尾添加一个图层，并命名为 `THRM Anchor`。
2. 保持图层文本为空，并将文字大小设置为 `1%`。
3. 将它移动到希望显示转速的位置，确认该图层位于图层列表最末尾，然后保存布局。

空图层仍会保留 RTSS 的图层边界，因此 `Cooler Fan` 可以独立于前一个监控项目移动。`1%` 的文字大小只用于压缩 RTSS 在槽位之间保留的空行，不会改变转速文本的字号。以后新增其他图层时，应继续让 `THRM Anchor` 保持在列表末尾。
